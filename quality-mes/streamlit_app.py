...
def log_activity(db, action, module, description):
    log = ActivityLog(user_id=st.session_state.user_id, username=st.session_state.username, action=action, module=module, description=description)
...
def dashboard_page():
    ...
    col2.metric("Dat", total_pass, delta=None)
    col3.metric("Khong dat", total_fail, delta=None, delta_color="inverse")
    ...
    if overdue_ncr > 0 or overdue_capa > 0:
        st.warning(f"Canh bao: {overdue_ncr} NCR va {overdue_capa} CAPA qua han!")

    st.divider()
    modules = [
        {"name": "IQC - Kiem tra dau vao", "icon": "📥", "desc": "Kiem tra chat luong nguyen vat lieu", "stat": f"{iqc_total} phieu ({iqc_pass} dat, {iqc_fail} khong dat)"},
        {"name": "IPQC - Kiem tra qua trinh", "icon": "⚙️", "desc": "Kiem tra chat luong trong san xuat", "stat": f"{ipqc_total} phieu ({ipqc_pass} dat, {ipqc_fail} khong dat)"},
        {"name": "OQC - Kiem tra thanh pham", "icon": "📤", "desc": "Kiem tra chat luong san pham cuoi", "stat": f"{oqc_total} phieu ({oqc_pass} dat, {oqc_fail} khong dat)"},
        {"name": "NCR + CAPA", "icon": "⚠️", "desc": "Bao cao su khong phu hop & khac phuc", "stat": f"{open_ncr} NCR dang mo"},
        {"name": "Thiet bi do", "icon": "🔧", "desc": "Quan ly thiet bi & hieu chuan", "stat": ""},
        {"name": "Nguoi dung", "icon": "👥", "desc": "Quan ly tai khoan", "stat": ""},
    ]
    for i, m in enumerate(modules):
        with st.container(border=True):
            st.subheader(f"{m['icon']} {m['name']}")
            st.caption(m['desc'])
            if m['stat']: st.caption(m['stat'])
...
def material_tab(db):
    ...
    with st.form("material_form"):
        ...
        sup_name = cols[4].selectbox("NCC", options=list(sup_map.keys()))
        if st.form_submit_button("Them NL", type="primary"):
            if code and name and sup_map:
...
def iqc_inspection_tab(db):
    ...
    mat_to_sup = {m.id: m.supplier_id for m in materials}
    suppliers = db.query(Supplier).order_by(Supplier.name).all()
    sup_map = {s.name: s.id for s in suppliers}
    ...
    with st.expander("Tao phieu kiem tra moi", expanded=False):
        with st.form("iqc_insp_form"):
            ...
            mat_name = cols[1].selectbox("Nguyen lieu", options=list(mat_map.keys()))
            sup_name = cols[2].selectbox("Nha cung cap", options=list(sup_map.keys()))
            ...
            insp_name = cols2[1].selectbox("Nguoi kiem", options=list(user_map.keys()))
            notes = cols2[2].text_input("Ghi chu")
            if st.form_submit_button("Tao phieu", type="primary"):
                if insp_no and mat_name and lot:
    ...
    inspections = db.query(IQCInspection).order_by(IQCInspection.created_at.desc()).limit(50).all()
    if inspections:
        data = []
        for insp in inspections:
            data.append({"ID": insp.id, "So phieu": insp.inspection_no, "Nguyen lieu": insp.material.name if insp.material else "", "Lo": insp.lot_no, "Trang thai": STATUS_LABELS.get(insp.status, insp.status), "Ngay": str(insp.inspection_date)[:10]})
            if len(data) == 1:
                with st.expander(f"Chi tiet {insp.inspection_no} - {STATUS_LABELS.get(insp.status, insp.status)}", expanded=False):
                    st.write(f"**Nguyen lieu:** {insp.material.name if insp.material else 'N/A'}")
                    st.write(f"**NCC:** {insp.supplier.name if insp.supplier else 'N/A'}")
                    st.write(f"**So lo:** {insp.lot_no} | **SL/Sample:** {insp.quantity}/{insp.sample_size}")
                    results = db.query(IQCResult).filter(IQCResult.inspection_id == insp.id).all()
                    if results:
                        rdf = pd.DataFrame([{"Muc kiem": r.item_name, "Tieu chuan": r.specification or "", "Gia tri do": r.measured_value, "Min": r.standard_min, "Max": r.standard_max, "Ket qua": "Dat" if r.result == "pass" else "Khong dat"} for r in results])
                        st.dataframe(rdf, use_container_width=True, hide_index=True)
                    if insp.status == "pending":
                        c1, c2 = st.columns(2)
                        if c1.button("Dat", key=f"pass_{insp.id}"):
                            insp.status = "pass"; db.commit(); st.rerun()
                        if c2.button("Khong dat", key=f"fail_{insp.id}"):
                            insp.status = "fail"; db.commit(); st.rerun()
                    with st.form(f"add_result_{insp.id}"):
                        c1, c2, c3, c4 = st.columns(4)
                        item = c1.text_input("Muc kiem")
                        measured = c2.number_input("Gia tri do", value=0.0, format="%.3f")
                        min_v = c3.number_input("Min", value=0.0, format="%.3f")
                        max_v = c4.number_input("Max", value=0.0, format="%.3f")
                        if st.form_submit_button("Them ket qua"):
                            if item:
                                ...
                                db.commit()
                                st.success("Them ket qua thanh cong"); st.rerun()
    ...
    with tab1:
        ...
        with st.expander("Tao phieu kiem tra moi", expanded=False):
            with st.form("oqc_insp_form"):
                ...
                if st.form_submit_button("Tao phieu", type="primary"):
                    if insp_no and prod_name and lot:
        ...
        inspections = db.query(OQCInspection).order_by(OQCInspection.created_at.desc()).limit(50).all()
        if inspections:
            data = []
            for insp in inspections:
                data.append({"ID": insp.id, "So phieu": insp.inspection_no, "San pham": insp.product.name if insp.product else "", "Lo": insp.lot_no, "Trang thai": STATUS_LABELS.get(insp.status, insp.status), "Ngay": str(insp.inspection_date)[:10]})
                if len(data) == 1:
                    with st.expander(f"Chi tiet {insp.inspection_no}", expanded=False):
                        ...
                        with st.form(f"oqc_add_res_{insp.id}"):
                            c1, c2, c3, c4 = st.columns(4)
                            item = c1.text_input("Muc kiem")
                            measured = c2.number_input("Gia tri do", value=0.0, format="%.3f")
                            min_v = c3.number_input("Min", value=0.0, format="%.3f")
...
def ipqc_page():
    ...
    with st.expander("Tao phieu kiem tra moi", expanded=False):
        with st.form("ipqc_insp_form"):
            ...
            insp_name = c8.selectbox("Nguoi kiem", options=list(user_map.keys()))
            if st.form_submit_button("Tao phieu", type="primary"):
                if insp_no and process and wc:
    ...
    inspections = db.query(IPQCInspection).order_by(IPQCInspection.created_at.desc()).limit(50).all()
    if inspections:
        for insp in inspections:
            with st.expander(f"{insp.inspection_no} - {insp.process_name} ({STATUS_LABELS.get(insp.status, insp.status)})"):
                ...
                with st.form(f"ipqc_add_res_{insp.id}"):
                    c1, c2, c3, c4 = st.columns(4)
                    item = c1.text_input("Muc kiem")
                    measured = c2.number_input("Gia tri do", value=0.0, format="%.3f")
                    min_v = c3.number_input("Min", value=0.0, format="%.3f")
...
def ncr_page():
    ...
    with tab1:
        with st.expander("Tao NCR moi", expanded=False):
            with st.form("ncr_form"):
                ...
                c6, c7 = st.columns(2)
                assignee = c6.selectbox("Nguoi xu ly", options=[""] + list(user_map.keys()))
                ...
                if st.form_submit_button("Tao NCR", type="primary"):
                    if ncr_no and title:
                        ...
                        if exist: st.error("So NCR da ton tai")
                            ...
                            n = NCR(ncr_no=ncr_no, title=title, description=desc, severity=severity, source_type=source_type, reported_by=user_map[reporter], assigned_to=user_map[assignee] if assignee else None, due_date=datetime.combine(due, datetime.min.time()) if due else None)
                            db.add(n); db.commit()
        ...
        for n in ncrs:
            with st.expander(f"{n.ncr_no} - {n.title} ({NCR_STATUS.get(n.status, n.status)})"):
                ...
                with st.form(f"capa_form_{n.id}"):
                    ...
                    c1, c2 = st.columns(2)
                    capa_no = c1.text_input("So CAPA")
                    capa_title = c2.text_input("Tieu de CAPA")
                    capa_desc = st.text_area("Mo ta CAPA")
                    c3, c4, c5 = st.columns(3)
                    capa_type = c3.selectbox("Loai", ["corrective", "preventive"], format_func=lambda x: "Khac phuc" if x == "corrective" else "Phong ngua")
                    capa_assignee = c4.selectbox("Nguoi phu trach", options=[""] + list(user_map.keys()))
                    capa_due = c5.date_input("Han hoan thanh")
                    root_cause = st.text_input("Nguyen nhan goc")
                    action = st.text_area("Ke hoach hanh dong")
                    if st.form_submit_button("Them CAPA"):
                        if capa_no and capa_title:
                            ca = CAPA(capa_no=capa_no, ncr_id=n.id, type=capa_type, title=capa_title, description=capa_desc, root_cause=root_cause, action_plan=action, assigned_to=user_map[capa_assignee] if capa_assignee else None, due_date=datetime.combine(capa_due, datetime.min.time()) if capa_due else None)
                            db.add(ca); db.commit()
    ...
    with tab2:
        ...
        if all_capas:
            data = []
            for c in all_capas:
                data.append({"So CAPA": c.capa_no, "Tieu de": c.title, "Loai": "Khac phuc" if c.type == "corrective" else "Phong ngua", "Status": CAPA_STATUS.get(c.status, c.status), "Han": str(c.due_date)[:10] if c.due_date else ""})
            st.dataframe(pd.DataFrame(data), use_container_width=True, hide_index=True)
...
def spc_page():
    ...
    with tab2:
        ...
        if results:
            ...
            fail_df = pd.DataFrame([{"Muc kiem": r.item_name} for r in results if r.result == "fail"])
            if not fail_df.empty:
                fail_counts = fail_df.groupby("Muc kiem").size().reset_index(name="So loi")
...
def equipment_page():
    ...
    if equipments:
        for eq in equipments:
            ...
            status_icon = "🔴" if overdue else ("🟠" if days_left and days_left <= 30 else "🟢")
            with st.expander(f"{status_icon} {eq.code} - {eq.name}"):
                c1, c2, c3 = st.columns(3)
                c1.write(f"**Loai:** {eq.type}")
                c2.write(f"**Vi tri:** {eq.location or 'N/A'}")
                c3.write(f"**Chu ky:** {eq.calibration_interval_days} ngay")
                c4, c5 = st.columns(2)
                c4.write(f"**Hieu chuan cuoi:** {eq.last_calibration_date}")
                c5.write(f"**Han hieu chuan:** {eq.next_calibration_date} {'(QUA HAN!)' if overdue else f'(con {days_left} ngay)' if days_left else ''}")
                with st.form(f"cal_{eq.id}"):
                    cal_date = st.date_input("Ngay hieu chuan", value=today)
...
def users_page():
    ...
    with st.expander("Them nguoi dung", expanded=False):
        with st.form("add_user_form"):
            c1, c2 = st.columns(2)
            username = c1.text_input("Ten dang nhap")
            email = c2.text_input("Email")
            c3, c4 = st.columns(2)
            full_name = c3.text_input("Ho ten")
            role = c4.selectbox("Vai tro", options=list(ROLES.keys()), format_func=lambda x: ROLES[x])
